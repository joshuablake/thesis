...
This chapter aims to estimate the duration of SARS-CoV-2 PCR positivity from CIS data if perfect testing can be assumed (i.e.: no misclassification bias).
...
Series of positive tests within CIS were heuristically categorised into episodes by Sarah Walker (see \autoref{episode-def}).
These tests are assumed to have been due to a single infection event.
...
The \emph{initiating event} is when the duration starts, here becoming detectable.
The \emph{terminating event} is when the duration starts, here becoming no longer detectable.

The reason for this approach, over modelling biomarkers (as in chapter\todo{ref chapter on ATACCC}) is...
\todo[inline]{Justify approach}

The methods in this chapter deal with modelling two issues that arise from the data.
...
\begin{figure}
  \todo[inline]{Copy over figure demonstrating double interval censoring}
  \caption{Episodes data in CIS is double interval censored, meaning that both the start and end of the episode are only known up to an interval. \label{perf-test:fig:double-interval-censor}}
\end{figure}
...
Truncation occurs because we do not observe some episodes: an episode can occur between tests and hence the episode is never detected (see figure\todo{truncation figure}).
We do not know how 

This combination of issues are rarely encountered in biomedical data\todo{refs to combining double interval censoring with truncation}. 
...
\item
  ...
  16th Oct).
\item
  A negative test to end the episode (i.e.: following the final positive in the episode) has been recorded.
\end{itemize}
...
recovery (i.e.: are strict subsets of $T \times T$) given the
...
Outside this period, all tests are negative (i.e.: no false positives occur). 
