...
The model here is an application of a standard model for viral dynamics, used for many diseases\todo{cite history of viral load ODE models}.
...
\todo[inline]{Check if the $\mu$ parameter priors are on the mean of log parameters or mean of parameters}
\end{table}
...
The vaccinated population have shorter durations of RT-PCR positivity\todo{check posterior probability that vaccinated population have shorter durations} than the unvaccinated population (see \cref{ATACCC:fig:duration}).
...
\todo{Do the impact of these factors agree with other estiamtes? Does immunity matter more than other factors?}
