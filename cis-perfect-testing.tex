...
Infrequent testing complicates analysis of the data in two ways.
...
I base my analysis on the 4,800 episodes first detected between 10th Oct 2020 and 6th Dec 2020 inclusive with negatives bounding the start and end time of the episode.
...
They apply it to a situation where the visit times (possible detection times) are global, simplifying the likelihood.
...
I developed a novel Bayesian framework without explicit data augmentation.
...
This framework is itself based on long-standing methodology~\autocite{dempsterMaximum,turnbullEmpirical}.
...
All the relevant information about episode $i$ can be fully characterized by the (unobserved) triplet $(b_j, e_j, \sched_j)$ where $b_j$ and $e_j$ are the times the episode being and end respectively and $\sched_j$ is the set of test times for the individual in which the episode occurs.
...
and 28. Inadmissible regions are unshaded. The black (impossible) region is because it does not make sense for recovery to occur prior to infection. \label{perf-test:fig:partitionSpace}}
\end{figure}
...
I multiply these probabilities by $\eta_i/\Ncis$, where $\eta_i$ is the number of individuals with test schedule $\sched_i$, to give the probability that an episode belongs to each class, without conditioning on a specific individual.
...
\begin{figure}
  ...
  \caption[RW2 prior for the hazard]{The prior predictive distribution (mean and 95\% central interval) for the RW2 prior specified in the main text. The posterior mean estimate using ATACCC data in \cref{E-ATACCC} are shown for comparison. \label{perf-test:fig:rw2-prior}}
\end{figure}
