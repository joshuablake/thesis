...
A posterior sample for $\vec{P}$ can be transformed to a posterior sample for $\vec{Z}$ based on \cref{E-inc-prev:eq:forward-substitute}, see \cref{backcalc:sec:methods} for details.
...
The algorithm is based on \cref{E-inc-prev:eq:forward-substitute}, which is repeated here for convenience:
\begin{align}
Z_t
...
\Cref{transmission:eq:forward-substitute} is derived from \cref{E-inc-prev:eq:EPt-matrix}: $\vec{P} = \matr{S} \vec{I}$.
...
\begin{figure}
    ...
    \caption[Incidence estimated using backcalculation by region]{%
        ...
        Yorkshire and the Humber is labelled Yorkshire for brevity.
    }
