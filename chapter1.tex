...
previous studies, such as \protect\hyperlink{ref-heiseyModelling}{Heisey
and Nordheim} (\protect\hyperlink{ref-heiseyModelling}{1995}),
\protect\hyperlink{ref-dempsterMaximum}{Dempster, Laird, and Rubin}
(\protect\hyperlink{ref-dempsterMaximum}{1977}), and
\protect\hyperlink{ref-turnbullEmpirical}{Turnbull}
(\protect\hyperlink{ref-turnbullEmpirical}{1976}). The conceptual
...
Unlike most Bayesian methods (eg:
\protect\hyperlink{ref-heBayesiana}{He}
(\protect\hyperlink{ref-heBayesiana}{2003}),
\protect\hyperlink{ref-heBayesian}{He and Sun}
(\protect\hyperlink{ref-heBayesian}{2005}), and
\protect\hyperlink{ref-caoModeling}{Cao et al.}
(\protect\hyperlink{ref-caoModeling}{2009})) we avoid augmenting the
...
(\textgreater4000) making this approach computationally expensive.
...
\item
  ...
  \(Pr(D_i \geq t \mid B_i = b, \theta) = Pr(D_i \geq t \mid \theta) = S_\theta(t)\),
...
Formally (and following \protect\hyperlink{ref-heiseyModelling}{Heisey
and Nordheim} (\protect\hyperlink{ref-heiseyModelling}{1995})), we
...
\item
  ...
  \(p_{ia} = Pr((b, e) \in \alpha_i \mid \theta)\).
\item
  ...
  \(p_{it} = Pr((b, e) \in \Omega^C_i \mid \theta)\).
\item
  ...
  \(p_{iu} = Pr((b, e) \in \beta_i \mid \theta)\).
\end{itemize}
...
consistency with \protect\hyperlink{ref-heiseyModelling}{Heisey and
Nordheim} (\protect\hyperlink{ref-heiseyModelling}{1995})), and is the
complement of \(\Omega^C_i\).

\begin{figure}
\centering
\includegraphics{20230216_123954.jpg}
...
\hypertarget{solution-for-the-sum}{%
...
parameters (\protect\hyperlink{ref-dempsterMaximum}{Dempster, Laird, and
...
(\protect\hyperlink{ref-gelmanBayesian}{2013}) section 8.7.5), and is
the reference prior for this quantity
(\protect\hyperlink{ref-heBayesiana}{He}
(\protect\hyperlink{ref-heBayesiana}{2003})). Under this prior,
...
&= \frac{\Gamma(a+1)b^a}{\Gamma(a) (b+1-p_{it})^{a+1}} \int \frac{(b+1-p_{it})^{a+1}}{\Gamma(a+1)} \lambda^{(a+1)-1} e^{-(b+1-p_{it})\lambda} d\lambda \\
...
\hypertarget{derivations-of-probabilities}{%
\subsubsection{\texorpdfstring{Derivations of probabilities
\label{sec:deriv-probs}}{Derivations of probabilities }}\label{derivations-of-probabilities}}

By definition, we have \(p_{ia} = Pr((b, e) \in \alpha_i)\) and
\(\alpha_i = \{ (b, e) : l_i^{(b)} \leq b \leq r_i^{(b)} \wedge l_i^{(e)} \leq e \leq r_i^{(e)}\}\).
Hence: \[
\begin{aligned}
p_{ia}
&= Pr \left( l_i^{(b)} \leq B_i \leq r_i^{(b)}, l_i^{(e)} \leq E_i \leq r_i^{(e)} \right) \\
&= Pr \left( l_i^{(e)} \leq E_i \leq r_i^{(e)} \mid l_i^{(b)} \leq B_i \leq r_i^{(b)} \right) Pr \left( l_i^{(b)} \leq B_i \leq r_i^{(b)} \right) \\
&=\sum_{b = l_i^{(l)}}^{r_i^{(b)}} Pr \left( l_i^{(e)} \leq E_i \leq r_i^{(e)} \mid B_i = b \right) Pr \left(B_i = b \right) \\
&=\sum_{b = l_i^{(b)}}^{r_i^{(b)}} Pr \left( l_i^{(e)} - b + 1 \leq D_i \leq r_i^{(e)} - b + 1 \right) Pr \left(B_i = b \right) \\
&=\sum_{b = l_i^{(b)}}^{r_i^{(b)}} \left( S_\theta(l_i^{(e)} - b + 1) - S_\theta(r_i^{(e)} - b + 2) \right) Pr \left(B_i = b \right) \\
...
Next, we derive \(p_{it} = Pr((b, e) \in \Omega^C_i)\), the probability
...
\(t^N_{it} = \min \{ t' \in t_i : t' \geq t \} - t\). Then
\(\Omega^C_i = \{ (b, e) : \nexists t \in t_i. b \leq t \leq e \vee b \leq \min(t_i) \} = \{ (b, e) : e - b < t^N_{ib} \vee b \leq \min(t_i) \}\).
Hence: \[
\begin{aligned}
1 - p_{it}
&= 1 - Pr(E_i - B_i < t_{iB_i}^N \vee B_i \leq \min(t_i)) \\
&= 1 - Pr(E_i - B_i < t_{iB_i}^N \wedge B_i > \min(t_i)) - Pr(B_i \leq \min(t_i)) \\
&= 1 - \sum_{b=\min(t_i) + 1}^T Pr(E_i - b + 1 < t_{ib}^N \mid B_i = b) Pr(B_i = b) - \sum_{t=1}^{\min(t_i)} Pr(B_i = b)\\
...
such that: \[
Pr(y_i(t) = 1) = \begin{cases} p_\text{sens} &b_i \leq t \leq e_i \\ 0 &\text{otherwise.}\end{cases}
\]
...
\hypertarget{modifying-p_ia}{%
\subsection{\texorpdfstring{Modifying
\(p_{ia}\)}{Modifying p\_\{ia\}}}\label{modifying-p_ia}}
...
&= p(y_i', l_i^{(e)} \leq e_i \leq r_i^{(e)} | t_i, b_i, p_\text{sens}, \theta) &\text{as no false pos} \\
...
Second, if \(e_i > r_i^{(e)}\). In this case, the test test at
...
\hypertarget{modifying-p_it}{%
\subsection{\texorpdfstring{Modifying
\(p_{it}\)}{Modifying p\_\{it\}}}\label{modifying-p_it}}
