...
Any infection episode, whether detected or not, is fully characterised for my purposes by the triplet $(b_j, e_j, i(j))$ where $b_j$ and $e_j$ are the times the episode begin and end respectively, and $i(j)$ is the individual the episode occurred in.
...
\intertext{and, for all $i$,}
p_{ia} + p_{iu} + p_{i\bar{a}} &= 1.
\end{align}
...
This issue can be rectified be considering the inadmissible region as many subregions each with an observed count of zero (sketched in \cref{perf-test:sec:alt-likelihood}).
Count zero regions do not contribute to the probability meaning the result is the same.
A formalisation of this approach requires much more complex notation for the same result; therefore, I prefer this presentation.
...
Therefore, the conditional probabilities are multiplied by $1/\Ncis$ to form the unconditional probabilities.
For example, the probability of an episode being in class $\alpha_i$ is $p_{ia} / \Ncis$.

An episode occurs in one of the regions independently and with the same probability.
Therefore:
\begin{align}
  \begin{bmatrix}
...
The third condition can be expressed as $\tau_{\sched_{i(j)}}(b_j) < \infty$.
