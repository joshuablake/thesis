...
Therefore, model assumptions determined the size of the tail estimated.
In this chapter, I develop a method to estimate the distribution using CIS data. 
CIS sampling is less frequent, but with a large sample size and long follow-up.
...
The CIS tests each individual infrequently, either weekly or monthly (see\todo{refer to section with CIS intro}).
...
The rest of the chapter develops a method to model them.
...
If the duration was 24 days or more, the individual would still be detectable at their next test and detected then.
CIS has many undetected infections. This is because frequency of testing (up to 28 days, see\todo{ref section on the survey design}) is longer than most infections (median around 15 days).
\begin{figure}
...
I base my analysis on the 4,800\todo{check number} episodes that were first detected between 10th Oct 2020 and 6th Dec 2020 inclusive with negatives bounding the start and end time of the episode.
...
Therefore, the probability of an infection episode being detected conditional on being infected is higher than the overall probability of being detected.\todo{this is a bit unclear, try and rephrase?}
...
Neither have theoretical justification~\autocite[230]{sunAnalysis}\todo{Try and understand why this is the case}.
...
\todo{Not sure if the followin paragraph is understandable}
...
Now I derive an analytical solution to $p(\ntot \mid \theta) = \sum_{\ntot=\ndet}^\infty \frac{\ntot!}{(\ntot - \ndet)!} \pnodet^{\ntot - \ndet}$ under the prior $\ntot \dist \NegBin(\mu, r)$ (using the mean/dispersion parameterisation of the negative binomial).
Equivalently:
\begin{align}
...
I use a second-order random walk because it is simple and produces plausible prior predictive distributions.
...
\todo[inline]{Add RW2 prior predictive}
...
Convergence was assessed using Rhat and ESS as described in \cref{E-ATACCC:sec:convergence}\todo{add ref to place explaining diagnostics}, and all runs checked for divergent transitions.
...
For hazards after day X\todo{check and insert}, the informative and vague priors are the same.
