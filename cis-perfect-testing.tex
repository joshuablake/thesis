...
\begin{itemize}
\item  I introduce the motivating dataset in section~\ref{cpt:sec:data}. This is the CIS dataset previously encountered, processed to form "episodes" of infection.
...
\section{Data}\label{cpt:sec:data}
...
\section{Modelling the duration}\label{cpt:sec:model}
...
\subsection{Individual model}\label{cpt:sec:individual-model}

\subsubsection{Likelihood}\label{cpt:sec:likelihood}
...
\ref{cpt:fig:partitionSpace}). Each episode, whether observed or not, must
fall into one of these three classes with a probability that is a
function of $\theta$ (see section \ref{cpt:sec:deriv-probs}).
...
$\beta_i$.\label{cpt:fig:partitionSpace}}
\end{figure}
...
\subsubsection{Full posterior}\label{cpt:sec:full-posterior}
...
\subsection{Total model}\label{cpt:sec:total-model}
...
\section{Parameterisation and priors for the survival function} \label{cpt:sec:parameters-priors}

\section{Simulation study} \label{cpt:sec:simulation-study}

\section{Conclusion} \label{cpt:sec:conclusion}
