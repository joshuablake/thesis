...
\todo{Maybe I need a better description of the swabbing process?}
...
First, it is well known that PCR testing can return false negatives \todo{refs for sensitivity} and I needed to explicitly include these in the viral load model of \cref{ATACCC}; they can be directly observed within CIS as intermittent negatives (see below).
Second, adjusting the simulation to include false negatives can reproduce both the CIS data more faithfully and the same issues when estimating the duration distribution (see \cref{imperf-test:sec:simulate})
...
\todo{Reference to ATCCCC chapter, although maybe the specific section with this result would be better}
...
\begin{figure}
  ...
  \caption{Bounding the test sensitivity using CIS data as a function of time since the infection was detected (see main text for explanation of how the bounds are formed).\label{imperf-test:fig:bounding-cis-sensitivity}}
\end{figure}
