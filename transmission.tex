...
However, except in an epidemic's early stages, the interaction between large regions has a negligible impact on the epidemic's progression~\autocite[e.g.][]{birrellBayesian,gogSpatial,eggoSpatial}.
...
That is, if $\vec{x}_t$ is the system state at time $t$ for $t \in \{ 0, 0.5, 1, \dots \}$ then $y_{t+\frac{1}{2}} = y_t + (d\vec{x}(t)/dt) \delta t$.
