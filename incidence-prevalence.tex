...
I then explain the three relevant models: one for the infection process (\cref{inc-prev:sec:infection-process}); one for the prevalence process, the relevant disease model (\cref{inc-prev:sec:prevalence-process}) linking infections to prevalence; and finally the observation process for a prevalence survey like the CIS (\cref{inc-prev:sec:observation-process}).
...
$P_t$ corresponds with $W_t$ in \cref{E-intro:eq:disease-model}.
...
The WAIFM allows calculation of the number of secondary infections generated by a single infection in the $a$th strata,  this follows directly from its definition as $\frac{1}{\gamma} \sum_{a' \in \set{A}} \beta_{a'a}$. 
