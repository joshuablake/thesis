...
&= \prob(\vec{\nu}_k, l_k^{(e)} \leq E_j \leq r_k^{(e)} \mid B_j = b, p_\text{sens}, \vec{\theta}) \\ % &\text{as no false positives}
...
\begin{align}
p_{ik}'
=& \prob(\vec{\nu}_k \mid p_\text{sens}, \vec{\theta}) \\
=& \sum_{b = l_k^{(b)}}^{r_k^{(b)}} \left( \prob(\vec{\nu}_k, E_j \leq r_k^{(e)} \mid B_j = b, p_\text{sens}, \vec{\theta}) + \prob(\vec{\nu}_k, E_j > r_k^{(e)} \mid B_j = b, p_\text{sens}, \vec{\theta}) \right) \\
