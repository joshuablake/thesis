...
The aim of this chapter is to estimate the duration of RT-PCR positivity using the viral load data from the ATACCC study (described in\todo{ref section describing ATACCC}).
...
The observation model then describes how the RT-PCR testing process produces noisy observations of this viral load.
The observation model includes the possibility of false positive and negatives, and the limit of detection of the RT-PCR test.
...
RT-PCR tests are noisy observations of the viral load.
The noise arises from a variety of sources, including the sampling/swabbing process and batch effects in the RT-PCR process~\autocite{hanRTPCR}.
A positive RT-PCR test has a Ct value.
Ct values are linearly related to viral load (see \cref{E-biology-data:sec:PCR})
A negative RT-PCR test signals either the viral load is too low to be detected, \ie below the limit of detection, or the test is a false negative.
Positive RT-PCR tests, especially those with low viral loads, could be false positives, for example caused by contamination.
...
Hence, their duration of RT-PCR positivity, in continuous time, is $d_i = \tau_{1,i} - \tau_{0,i}$.
