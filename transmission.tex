...
\begin{enumerate}
    \item The smoothing model, $g_Z$ or $g_P$, over time should penalise changes from continuous exponential growth of $\pi$ over time.
        ...
        Furthermore, such penalization has previously been shown to improve estimates of SARS-CoV-2 prevalence by balancing over and under-fitting~\autocite{ealesAppropriately}. Used by CIS (section 3.?).
...
Properties 1 and 2 suggest a model of the form $\logit(\pi_{i,t}) = g(i, t)$, where $g$ is a function with changes in the first derivative penalized w.r.t.\ $t$ (\ie changes away from linear are penalised).
