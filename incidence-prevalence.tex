...
Within this thesis, I define an individual as having SARS-CoV-2 on day $t$ if, had they been swabbed on day $t$, that swab would return a positive RT-PCR test.
...
\todo{check $B_i$ and $E_i$ are not defined elsewhere}
...
Stochastic models specify a probability distribution for the current state $p(\vec{x}(t) \mid \set{H}_t, \vec{\theta})$ where $\set{H}_t = \{ \vec{x}(t') \ssep t' < t \}$ is the epidemic's history and $\vec{\theta}$ are model parameters\todo{Cite stochastic models}.
...
The simplest compartmental model for a disease which confers immunity is the SIR (susceptible-infectious-recovered) model\todo{cite SIR being simplest} (depicted in \cref{SEIR:fig:SIR}).
The name SIR derives from the structure of the model.
...
\todo{Is a division by $N_a$ necessary here? Check previous notes on Overleaf including on what Edwin provides.}
...
\todo[inline]{Does something need to link these sections?}
...
\todo[inline]{Relate the following paragarph to the similar discussion in ch 7}
...
Stan implements \emph{automatic differentiation}\todo{cite automatic diff} to calculate the gradient of the posterior density without any user input.
...
However, no estimates of this quantity in the general population exist\todo{ref lit review of duration}.
