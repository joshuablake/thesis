...
Of particular interest is that the basic reproduction number, $R_0$, and the time-varying reproduction number, $R_t$, can both be calculated as simple functions of the model parameters; in \cref{E-backcalc} additional modelling is required to calculate these numbers.
...
In general, this means, there is a dependence between $\vec{x}(t)$ and $\vec{x}(t')$.
...
When the number of infected individuals is large, then this mean-field behaviour is a good approximation of the fully stochastic model, due to the law of large numbers\todo{ref mean-field and approximation error}.
...
This argument is similar to the justification for neglecting the stochasticity of the prevalence process in \cref{E-inc-prev:sec:observation-process}.
...
\Cref{SEIR:sec:SIR} begins by introducing the \emph{susceptible-infected-recovered} (SIR) model, a simple compartmental model for a disease which confers immunity.
...
This model has several assumptions which make it inappropriate to model SARS-CoV-2; therefore, I introduce extensions to relax these assumptions.
...
Next, \cref{SEIR:sec:structured-populations} relaxes the homogeneous population assumption, introducing stratification by important characteristic(s).
...
Individuals in the susceptible compartment can be infected; $s(t)$ is the proportion of the population that is susceptible at time $t$.
Individuals in the infectious compartment can infect others, and eventually recover; $i(t)$ is the proportion of the population that is infectious at time $t$.
Individuals in the recovered compartment are immune to the disease, and cannot be infected again; $t(t)$ is the proportion of the population that have recovered by time $t$.
...
These parameters are assumed to be constant (an assumption relaxed in \cref{SEIR:sec:time-varying-foi}).
...
This term can be arrived at by considering a single infected individual~\autocite[214]{kretzschmarMathematical}.
Assume this individual comes into contact with other individuals in the population at a rate of $\kappa$ per day.
Further, assume the population is \emph{well-mixed}, meaning that each contact is with an individual chosen uniformly at random from the population.
...
Combined with the above, the expected rate of infections caused by this infected individual is $\kappa s q$.
...
Multiplying by the number of infected individuals and dividing by the population size gives the rate at which susceptible individuals are infected as a proportion of the population, $\beta s(t) i(t)$.
...
\begin{itemize}
    \item The basic reproduction number (discussed previously in\todo{ref intro}) for this model is $R_0 = \beta / \gamma$~\autocite[20]{keelingModeling}.
    ...
    \item There is no analytical solution to the system of ODEs~\autocite[25]{keelingModeling}: numerical methods are required to solve the system.
    ...
    \item Immunity is assumed to last until the end of the modelled period.
    For many diseases, including SARS-CoV-2, this is a reasonable assumption as long as the modelled period is short, as it will be in this chapter\todo{ref that SARS-CoV-2 immunity is long enough to not be an issue}.
    For modelling longer time periods, waning immunity can be incorporated\todo{ref waning immunity models}.
\end{itemize}
...
This parameterisation is chosen to give $1/\gamma$ as the mean infectious period (the gamma distribution and its expectation is defined in \cref{E-distributions}).
...
\todo{perhaps link with where define phases of infection in background chapters}
...
I will assume that the disease progression (\eg length of the exposed and infectious periods) is the same for all strata, although relaxing this assumption is easy.
...
Denote the set of strata of size $A$, as $\set{A}$.
A stratum will be denoted by $a \in \set{A}$ whose total population is $N_a$.

First, the state vector is now a vector of vectors, one for each of the compartments.
For example, in the SEIR model of \cref{SEIR:sec:SEIR}, $\vec{x} = (\vec{s}, \vec{e_1}, \dots, \vec{e_m}, \vec{i_1}, \dots, \vec{i_n}, \vec{r})^T$.
The $a$th element of each vector is the proportion of strata $a$ in that compartment.
For example, $\vec{s} = (s_1, \dots s_A)^T$ where $s_a$ is the proportion of the $a$th strata that are susceptible.
Hence, $N_a s_a$ is the number of susceptible people in stratum $a$.
These vectors simply replace the scalars in the previous equations.
Various other definitions are possible and equivalent, for example defining $s_a$ as the proportion of the total population that are both in strata $a$ and susceptible or parameterised using numbers of individuals\todo{cite Keeling}.

Second, the interactions between strata must be described.
This is done by replacing the scalar $\beta$ with a \emph{Who Acquires Infection From Whom} (WAIFW) matrix, $\matr{\beta}$.
The force of infection is now a vector, $\vec{\lambda} = \matr{\beta} \sum_{j=1}^n \vec{i_j}$.
The matrix element $\beta_{aa'}$ is the rate at which individuals in stratum $a$ infect individuals in stratum $a'$.
In practical situations, estimating all of these parameters is infeasible, so some assumptions must be made to reduce the number of parameters.
Common assumption include assuming symmetry, \ie $\beta_{aa'} = \beta{a'a}$ or a decomposable independence, \ie $\beta_{aa'} = \beta \beta^\text{inf}_a \beta^\text{susc}_{a'}$~\todo{ref A+M on these parameterisations}.
...
\todo[inline]{start with a generic intro on structured populations and use of matrices, then give age as a motivating example, finally specifics go elsehwere}
...
\todo[inline]{remove this section and just explain what I'm doing in this case in an implementation section}

To allow for regional hetereogeneity....
This assumes non-interaction between regions...
...
\todo[inline]{this paragraph probably just goes and its obvious from context}
...
\todo[inline]{need more explanation here: what is a phase-type distribution, be less terse in the following text}
...
\todo[inline]{mention that the model is run independently for each region, as in the previous chapter}
\todo[inline]{maybe put something about the likelihood here}
