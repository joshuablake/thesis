...
\todo[inline]{Consider moving all of this section into chapter 2, where CIS is introduced}
...
\todo[inline]{Think about the framing of this paragraph, maybe talk about being linear in its parameters and pre-existing software packages.}
The advantage of fitting a multilevel regression model on prevalence, rather than incidence, is that prevalence with appropriate prior (or penalisation) can be expressed as a generalised linear model (GLM)\todo{maybe cite backcalculation as a GLM}.
...
I draw 500\todo{check number of samples} samples from the approximate posterior distribution estimated.
...
\subsection{Methods}
\todo[inline]{add a bit more intro in the context of chapter 3}
...
I follow the WAIFW matrix parameterisation of \textcite{birrellRealtime}, decomposition $\beta_{aa'}(t) = \beta(t) \kappa_{aa'}(t) q_{aa'} N_{a'}$.\todo{remind readers what these quantities are here}
...
I link the modelled proportion of individuals that are PCR positive to the data using a beta-binomial likelihood (see\todo{ref section on clustering} for the issues with using a binomial likelihood).
