...
\todo[inline]{work out where to present full model equations and then edit/rearrange this section to reflect this}
...
The coverage of the posterior credible intervals should be their nominal value (\eg 95\% of the 95\% credible intervals should contain the true parameter value).
...
\todo[inline]{add simulation study results}
...
\todo[inline]{add peak incidence date plot}
...
\todo[inline]{add Rt results}

\todo[inline]{add attack rates and compare with an external data source}
\begin{figure}
...
The CIS allows data-informed estimates of transmission in children, unlike sources relying on severe events that occur almost entirely in adults\todo{cite something about mildness in children or age distribution of severe events}.

The parameters $\beta_t$ absorb all the time-varying factors not captured by Google mobility and school attendance data.
\todo{discuss the results and implications for policy or understanding transmission}
...
The decreases in $\beta_t$ over the autumn could be due to increased caution as cases and deaths increased (\eg masking or stricter adherence to social distancing)\todo{cite something about caution and reaction to rising cases, or survey data showing this happened - CoMix?}, or the impact of local measures in the highest transmission areas\todo{cite local measures}.
The increases in $\beta_t$ in December coincide with the rise of the Alpha variant~\autoref{lythgoeLineage}, known to be more transmissible~\autocite{daviesEstimated}.
...
Similarly, there is evidence that $q_c$ is different for Alpha and pre-Alpha variants\todo{cite a systematic review}, which could again motivate these changing over time.
