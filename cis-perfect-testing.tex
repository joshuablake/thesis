...
Bayesian methods have been used for nest survival before~\autocite{heBayesiana,heBayesian,caoModeling} by augmenting the data with the unobserved times.
...
$D_j$'s distribution is determined by $\vec{\theta}$; both $B_j$ and $i(j)$ are modelled as draws from uniform distributions over their respective discrete state spaces.
...
If $\posResults(W_j) = \emptyset$, $\min \posResults(W_j) \notin [1, T]$, or either $\negResults^{(b)}$ or $\negResults^{(e)}$ is undefined then the episode is undetected, in which case $O(j) = \emptyset$.
...
&\propto (b+1-\pnodet)^{-(a+\ndet)} \\
&= (r/\mu + 1 - \pnodet)^{-(r+\ndet)} &\text{substitute in $\mu$ and $r$}\\
...
=& \sum_{b = l_k^{(b)}}^{r_k^{(b)}} \prob \left( l_k^{(e)} - b + 1 \leq D_{j} \leq r_k^{(e)} - b + 1 \right) \prob \left(B_{j} = b \right) &\text{by definition of $D_{j}$} \\
=& \sum_{b = l_k^{(b)}}^{r_k^{(b)}} \left( S_{\vec{\theta}}(l_k^{(e)} - b + 1) - S_{\vec{\theta}}(r_k^{(e)} - b + 2) \right) \prob \left(B_{j} = b \right) &\text{by definition of $S_{\vec{\theta}}$} \\
...
The former, $\alpha = 0.5$, is Jefferys' prior~\autocite{jeffreysInvariant}, commonly considered an uninformative prior because it is invariant to the parameterization used~\autocite[53]{gelmanBDA}.
...
\begin{figure}
  \makebox[\textwidth][c]{\includegraphics{cis-perfect-testing/flat-prior}}
...
\begin{figure}
  \makebox[\textwidth][c]{\includegraphics{cis-perfect-testing/vague-prior}}
...
\begin{figure}
  \makebox[\textwidth][c]{\includegraphics{cis-perfect-testing/rw2-prior}}
...
Using a multivariate normal, as opposed to an independent univariate distribution for each $h_t$, preserves the correlation between the hazards.
...
\begin{figure}
  \thisfloatpagestyle{empty}
  % \vspace{-1cm}
  \makebox[\textwidth][c]{\includegraphics{cis-perfect-testing/ataccc-approximation-hazard}}
...
\end{figure}
\begin{figure}
  \makebox[\textwidth][c]{\includegraphics{cis-perfect-testing/ataccc-approximation-survival}}
...
\begin{figure}
  \makebox[\textwidth][c]{\includegraphics{cis-perfect-testing/kt-prior}}
...
\begin{figure}
  \makebox[\textwidth][c]{\includegraphics{cis-perfect-testing/input-duration-dists}}
...
\begin{figure}
  \makebox[\textwidth][c]{\includegraphics{cis-perfect-testing/survival-results}}
...
\begin{figure}
  \makebox[\textwidth][c]{\includegraphics{cis-perfect-testing/hazard-results}}
...
Individuals in the same household have both correlated infection times and correlated testing times, as they will typically have the same test schedule.
