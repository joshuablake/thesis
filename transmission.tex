...
\todo[inline]{
    ...
    Would include \textcite{mccabeCISincidence,abbottCISincidence} on backcalculation. Also Taffe?
...
\todo[inline]{This is now in the final section of chapter 2.}
...
% \end{enumerate}
% These groupings align closely with those previously used for analysis of the CIS data~\autocite[e.g.:][]{houseInferring}{walkerTracking}.
...
The method is based on MRP, a statistical methodology developed to correct political opinion polling suffering from response bias\todo{cite MRP}.
...
\todo{make sure ch 2 mentions strata explicitly}
...
\todo{make sure SRS computational issues are mentioned in ch 1 or 2}
Instead, previous MRP implementations have used the Integrated Nested Laplace Approximation (INLA)~\autocite{rueINLA}, implemented in R-INLA~\autocite{RINLA}\todo{add an appendix explaining INLA}.
...
\todo{check notation here doesn't clash with other notation}
...
Specifically, I modified the regression compomenet of a pre-existing version of a MRP model for CIS~\autocite{pouwelsMRPvaccination}, as recommended by that model's author for this situation~\citePersonalComms{Koen Pouwels}.
...
\todo{include what the default priors are (maybe in the appendix)}
...
\todo{include how R-INLA samples from the posterior in an appendix}
...
\Cref{E-inc-prev:eq:EPt}\todo{consider writing the matrix version where this is introduced in ch 3}, considered across different values of $t$, can be written as $\vec{P} = \matr{S} \vec{Z}$, where: $\vec{P}$ is a vector of the number of prevalent individuals at each time point in the stratum; $\matr{S}$ is a matrix formed from the survival function, as defined below; and $\vec{Z}$ is a vector of the incidence at each time point in the stratum.
...
\todo{Maybe move this paragraph to the discussion?}
...
There is weak evidence that when incidence starts increasing is correlated with when the Alpha variant's spread (see \cref{transmission:fig:backcalc-alpha}).
...
\todo[inline]{check discussing non-INLA approximations}
...
In \cref{E-SEIR:sec:structured-populations}, I explained how the elements of the WAIFW matrix, $\matr{\beta}$ can be decomposed as $\beta_{aa'} = \kappa_{aa'} q_{aa'} N_{a'}$ where\todo{should I repeat these definitions? they already exist in ch 3}: $\kappa_{aa'}$ is the mean rate at which an individual in stratum $a'$ makes contact with individuals in stratum $a$; $q_{aa'}$ is the probability of transmission upon contact between an infectious individual in stratum $a'$ and a susceptible individual in stratum $a$; and $N_{a'}$ is the total number of individuals in stratum $a'$.
...
The probability of transmission, $q_{aa'}(t)$, needs further decomposition to be identifiable, for the same issue of dimensionality preventing each element of $\matr{\beta}$ being estimated independently (see \cref{SEIR:sec:structured-populations} for a full explanation).
...
Separating relative transmissibility and susceptibility into two independent factors is a common approach\todo{cite anderson and may}.
...
I follow prior work~\autocite[e.g.][]{birrellRealtime}\todo{find other references for random walk on transmission potential} in using a random walk prior on $\log\beta(t)$  on the weekly timescale.
...
This idea extends the technique of having multiple exposed or latent compartments (introduced in \cref{E-SEIR:sec:non-exponential}), and is commonly used to enforce correct durations in compartmental models\todo{cite phase-type distributions in compartmental models}.
...
\todo[inline]{insert where the seroprevalence priors come from}
...
\todo{is $\pi_0$ unindentified because of $q_c$ - check correlation plot}
...
\todo{insert increase in attack rate over period}
\begin{figure}
...
\todo{Comment on SEIR not fully adjusted for non-response bias}
