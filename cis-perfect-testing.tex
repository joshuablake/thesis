...
Formally, the survival time function, parameterised by $\theta$, is the probability that an episode lasts at least $t$ days: $S_\theat(t) = \prob(D_i \geq t \mid B_i = b, \theta)$ where $D_i$ and $B_i$ are the duration and starting time of episode $i$ respectively.
...
The majority of this distribution is based on \cref{ATACCC}'s estimates from the ATACCC data.
