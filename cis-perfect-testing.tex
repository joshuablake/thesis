...
Flat priors can be unintentionally very informative, and hence uninformative priors should be used.
...
Through a simulation study, I show that these methods perform well, and in this context are not sensitive to model or whether the prior is vague or strongly informative (see \cref{perf-test:sec:simulation-study}).
...
\todo{better phrasing needed for this paragraph, mauybe try email to Shaun?}
...
$\ntot$ is generally a nuisance parameter, however, its posterior distribution can be useful for diagnostic purposes (\eg: see X\todo{ref where I use this in the next chapter}).
