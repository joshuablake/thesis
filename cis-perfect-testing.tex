...
\item
  ...
  If $i \in \set{D}$ then $n_{ia} =1$ admissible episodes occurred in individuals with test schedule $\sched{i}$, otherwise $n_{ia} = 0$.
...
Now I derive an analytical solution to $\sum_{\ntot=\ndet}^\infty p(\ntot \mid \theta) \frac{\ntot!}{(\ntot - \ndet)!} \pnodet^{\ntot - \ndet}$ under the prior $\ntot \dist \NB(\mu, r)$ (using the mean/dispersion parameterisation of the negative binomial, see \cref{E-distributions}).
