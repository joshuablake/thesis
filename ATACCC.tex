...
\todo{work out why Kissler citation is displaying weird}
...
The noisy observations of viral log load from RT-PCR testing are modelled by assuming each measurement of log viral load has an iid error distributed $\N(0, \sigma_v^2)$.
...
As the estimates are similar and the modified model converges better, I use only the results from the modified model from now on.
\begin{figure}
