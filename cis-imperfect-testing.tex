...
The new observation vector for $j$ is $O'(j) = [O(j), \vec{y}_j]^T$ where $\vec{y}_j$ is a binary vector of length $m_j$, with $y_{j,l}$ being the test result at time $t_{j,l}$.
...
As before, $p'_k = 1/\Ncis p'_{i_kk}$ and $p'_u = 1/\Ncis \sum_{i=1}^{\Ncis} p'{iu}$.

\subsection{Deriving $p'_{ik}$} \label{imperf-test:sec:modifying-p_ia}

I will modify $p_{ik}$ to form $p_{ik}' = \prob(O'(j) = \nu'_k \mid i(j) = i_k, \vec{\theta})$, which allows the negative test following the last positive to be a false negative.
...
This gives $t_+ = \sum_{l=1}^{m_k} y_{k,l}(t)$ true positives and $f_- = m_k - t_{k,+}$ false negatives.
...
I now modify $p_{iu}$ to form $p_{iu}' = \prob(O'(j) = \emptyset \mid i(j) = i_k, \vec{\theta})$, which allows the negative test following the last positive to be a false negative.
