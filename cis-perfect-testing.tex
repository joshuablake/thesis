...
%   $\prob(D_i \geq t \mid B_{j(i)} = b, \vec{\theta}) = \prob(D_i \geq t \mid \vec{\theta}) = S_{\vec{\theta}}(t)$,
...
\item
  ...
  Conditional on the episode occurring in individual $i$, the episode is admissible with probability $p_{ia} = \prob((b, e) \in \alpha_i \mid \vec{\theta})$.
...
\item
  ...
  Conditional on the episode occurring in individuals with test schedule $\sched_i$, the episode is undetected with probability $p_{iu} = \prob((b, e) \in \Omega_i \mid \vec{\theta})$.
\item
  ...
  Conditional on the episode occurring in these individuals, the episode inadmissible with probability $p_{i\bar{a}} = \prob((b, e) \in \beta_i \mid \vec{\theta})$.
\end{itemize}
...
\begin{align}
p_{ia} =& \prob((b, e) \in \alpha_i \mid \vec{\theta}) \\
\alpha_i =& \{ (b, e) : l_{j(i)}^{(b)} \leq b \leq r_{j(i)}^{(b)} \wedge l_{j(i)}^{(e)} \leq e \leq r_{j(i)}^{(e)}\}.
\intertext{Omitting the conditioning on $\vec{\theta}$, this gives:}
p_{ia}
...
=& \sum_{b = l_{j(i)}^{(b)}}^{r_{j(i)}^{(b)}} \left( S_{\vec{\theta}}(l_{j(i)}^{(e)} - b + 1) - S_{\vec{\theta}}(r_{j(i)}^{(e)} - b + 1) \right) \prob \left(B_{j(i)} = b \right) &\text{by def of $S_{\vec{\theta}}$} \\
\propto& \sum_{b = l_{j(i)}^{(b)}}^{r_{j(i)}^{(b)}} \left( S_{\vec{\theta}}(l_{j(i)}^{(e)} - b + 1) - S_{\vec{\theta}}(r_{j(i)}^{(e)} - b + 1) \right)
\end{align}
...
Therefore, the crucial component is $1 - p_{iu} = 1 - \prob((B_{j(i)}, E_{j(i)}) \in \Omega_i \mid \vec{\theta})$, one minus the probability that episode $i$ was undetected.
...
\end{align}
Therefore, suppressing the conditioning on $\vec{\theta}$:
\begin{align}
1 - p_{iu}
...
&= 1 - \frac{1}{T} \sum_{b=\min(\sched_i)+1}^T (1 - S_{\vec{\theta}}(\tau_{\sched_i}(b) + 1)) - \frac{1}{T} \min\sched_i \\
&= \frac{1}{T} \left(T - \sum_{b=\min\sched_i+1}^T (1 - S_{\vec{\theta}}(\tau_{\sched_i}(b) + 1)) - \min\sched_i \right) \\
&= \frac{1}{T} \left(T - (T - \min\sched_i) + \sum_{b=\min\sched_i+1}^T S_{\vec{\theta}}(\tau_{\sched_i}(b + 1)) - \min\sched_i \right) \\
&= \frac{1}{T} \sum_{b=\min(\sched_i)+1}^T S_{\vec{\theta}}(\tau_{\sched_i}(b) + 1). \label{perf-test:eq:piu}
\end{align}
