...
At the start of the PCR process, four substances are required: template DNA, nucleotides, primers, and DNA polymerase~\autocite{garibyanPCR}.
...
The choice of \emph{primers} specifies the exact DNA which will be amplified; they are the complementary DNA sequence to the start and end of the DNA sequence to be amplified~\autocite{garibyanPCR}.
...
In the second step, \emph{annealing}, the temperature is lowered to allow the primers to bind to the DNA.
In the final step, \emph{extension}, the temperature is raised, causing the DNA polymerase to replicate the DNA sequence between the primers.
...
Originally, the amount of target DNA was only measured after the completion of all cycles, hence the name real-time.
...
If there is more RNA present, proportionally more DNA is created in reverse transcription, and hence fewer cycles are required to reach detection; that is, the Ct value is lower~\autocite{hanRTPCR}.
The number of DNA copies doubles each cycle, therefore the log Ct value decreases linearly with the quanitiy of RNA or DNA in the original sample (see \cref{biology-data:fig:ct-calibration}).
