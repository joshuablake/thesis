...
Flat priors can be unintentionally very informative, and hence weakly informative priors should be used; in addition, the prior can be used to combine the information in the CIS with the results of \cref{ATACCC} (see \cref{perf-test:sec:parameters-priors}).
...
\todo[inline]{Need to discuss interpretation of the ghosts in the individual model, why this is hard, and the implications for setting a prior here. Not even clear in simulation what it should be!}
...
\todo{double interval censoring refs}.
\begin{figure}
...
Truncation is a large problem within CIS, especially when individuals move to the more infrequent testing schedule (described in \cref{intro:sec:cis}).
\todo{ref to description of CIS}
...
Therefore, unlike most Bayesian methods (e.g.: \textcite{heBayesiana}, \textcite{heBayesian}, and \textcite{caoModeling}) we avoid augmenting the data with unobserved times of infection or any information on the truncated infections; this would be computationally expensive with the large number of episodes in CIS (>4000 in the time period considered in the main analysis of \cref{imperf-test}).
...
\todo[inline]{Fix the impossible region to fill out the RHS; label the inadmissible regoin.}
...
This prior expresses extreme scepticism in estimates from previous studies, such as \cref{ATACCC} and the meta-analysis of \textcite{cevikShedding}.
...
The first step, approximating the \cref{ATACCC} posterior hazard as a multivariate normal on the logit scale, is done through the method of moments.
