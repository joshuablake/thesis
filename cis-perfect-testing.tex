...
False negatives are not dealt with and left to chapter X.

The first issue is double interval censoring.
An event (either the initiating or terminating) is \emph{interval censored} if its value is only to lie within a range.
\emph{Double interval censoring} means both the initiating and the terminating events are interval censored.
The initiating event is censored because we only observe the change from being not detectable (i.e.: negative) at one test, to detectable (i.e.: positive) at the following test; at the terminating event we see the opposite occurring (see\todo{interval censoring figure}).
...
\todo[inline]{Digitalise figure of regions.}
