...
The method is based on multilevel regression and post-stratification (MRP)\todo{cite MRP}.
...
Incorporating sufficient covariates to adjust for the non-response bias over a period of more than 6--8 weeks leads to a model that was too computationally expensive~\citePersonalComms{Koen Pouwels}.
Instead, previous MRP implementations have used the Integrated Nested Laplace Approximation (INLA)\todo{cite INLA}, implemented in R-INLA\todo{cite INLA}.
...
\Textcite{osogamiClosed} provide a method to construct the compartments, implemented in the mapfit R package~\autocite{mapfit} implements the expressions to calculate the EC distribution parameters given a distribution's first three moments.
...
\begin{table}
    ...
    \begin{tabular}{c c c c c}
        $l$ & $\omega_Y$ & $\omega_{X1}$ & $\omega_{X2}$ & $\pi$ \\
...
The mapfit R package~\autocite{mapfit} implements the expressions to calculate the EC distribution parameters given a distribution's first three moments.
...
\begin{tikzpicture}[
    ...
    \path  (S) edge node {$\lambda$} (P0)
          ...
          (P2) edge node {$\pi \omega_{X1}$} (P3)
               edge [out=-45,in=-135] node[below] {$(1 - \pi) \omega_{X1}$} (P4)
...
\begin{align}
    ...
    \frac{d\vec{p_3}(t)}{dt} &= \pi \omega_{X1} \vec{p_2}(t) - \omega_{X2} \vec{p_3}(t)
\end{align}
...
\begin{align}
    ...
    \vec{p_3} &= \vec{p_2} \frac{\pi \omega_{X1}}{\psi + \omega_{X2}}.
\end{align}
