...
\begin{figure}
  \makebox[\textwidth][c]{\includegraphics{cis-imperfect-testing/CIS_perfect}}
...
\begin{figure}
  \makebox[\textwidth][c]{\includegraphics{cis-imperfect-testing/sim-single-positive-episodes}}
...
Additionally, I limit the number of permutations of false negative and true positive tests that I consider have non-negligible probability.
Specifically, I assume that, if $i(j)$ is tested during infection episode $j$, either: the individual has a single false negative test and the episode ends before their next test, or the first test is a false positive test.
...
Introducing false negatives means that $O(j)$, containing the relevant information for episode $j$, is now random, even if $B_j$ and $E_j$ are known.
...
\begin{figure}
  \makebox[\textwidth][c]{\includegraphics{cis-imperfect-testing/CIS_final}}
...
\begin{figure}
  \makebox[\textwidth][c]{\includegraphics{cis-imperfect-testing/CIS_ntot}}
