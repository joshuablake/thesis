...
These classifications only describe the transmission model.
The observation model, which links $\vec{x}(t)$ to data, is generally includes some stochasticity regardless~\autocite{birrellEvidence}.
...
The basic reproduction number is then defined as $R_0 = D(\matr{\beta}) / \gamma$.
...
I use the following age-based strata, which align closely with the strata previously used for analysis of the CIS data~\autocites[e.g.:][]{houseInferring}{walkerTracking}.
\begin{enumerate}
...
\end{enumerate}
The CIS only recruited individuals aged 2 or older, however, I assume tests are representative of the whole $[0, 11)$ age group.
...
The interpretation of $q_c$ is that children have a susceptibility, relative to adults, of $q_c$.
...
\todo[inline]{sort region names in plots}
...
\begin{figure}
    ...
    \label{SEIR:fig:incidence}
    \todo[inline]{write peak incidence caption}
\end{figure}
