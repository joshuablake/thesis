...
Next, I describe the model of \textcite{hakkiOnset} in detail, which applies these methods to the ATACCC data (\cref{ATACCC:sec:hakki}) and before I modifying its observation model (\cref{ATACCC:sec:observation-modification})\todo{think if some justification is needed here, and if so how to do it - remember Hakki is a published and peer-reviewed analysis}.
...
\todo{Paul/Dani: is this phrasing "a realisation of [a] random variable" OK?}
...
\todo{this sentence is a bit clumsy...}
Therefore, the model for test failures can be simplified to have a constant probability of a false negative.
...
While the population-level parameters converged in the modified model, both models had convergence issues (Rhat > 1.4 and ESS < 20) for some individual-level parameters.
...
The individual trajectories are not of interest for this analysis, and the population-level parameters agree between the two sets of chains, therefore this should not affect any estimates of interest.
\todo{clunky phrasing...}
\begin{figure}
