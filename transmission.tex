...
Furthermore, fitting to just prevalence data has not previously been done in the literature (see\todo{ref lit review in intro})\todo{is using prevalence data novel or only in SARS-CoV-2 context?}.
...
The method is based on MRP, a statistical methodology developed to correct political opinion polling suffering from response bias~\autocite{gelmanMRT,gelmanPoststratication,parkMRP}.
...
\begin{itemize}
    \item $n_{taes}$ and $y_{taes}$ are the number of individuals tested and the number of positive test results on day $t$ in individuals of age group $a$, ethnicity $e$, and sex $s$. I use a beta-binomial distribution, as discussed in \cref{E-biology-data:sec:positivity}.
...
\end{itemize}
All parameters have the default R-INLA priors (see \cref{transmission:sec:INLA:priors}), except where stated above.
...
Having fitted the prevalence model, I draw a sample of 500 prevalence time series in each stratum from the approximate posterior distribution (using the procedure described in \cref{transmission:sec:INLA:posterior}).
...
\todo[inline]{check discussing non-INLA approximations, including how this would prevent negative incidence}
