...
Next, \cref{ATACCC:sec:hakki} explains \textcite{hakkiOnset}'s application of these methods to the ATACCC data and in Section \cref{ATACCC:sec:observation-modification} the observation model of Hakki is modified  to improve convergence.
...
The second approach is to approximate the trajectories from these models using general mathematical functions.
...
The viral load model produces a deterministic trajectory for the viral load of an individual, based on the model parameters.
The observation model then describes how the PCR testing process produces noisy observations of this viral load, including the possibility of false positive and negatives, and the limit of detection of the PCR test.
...
Viral load exhibits exponential growth followed by exponential decline.
...
As $|\tau - tau_\max|$ becomes large, the viral load trajectory tends towards exponential growth (before the peak) or decline (after the peak) in viral load at rates $a_i$ and $b_i$ respectively.
...
The noisy observations are modelled by assuming each measurement of log viral load is a realisation of a iid Normally-distributed random variable error with mean 0 and standard deviation $\sigma_v$.
...
p(y_{i,t} \mid \cdot) &= \begin{cases}
    ...
    \rho F_N(v_\text{lod} \mid x_0, \sigma_0^2) + (1 - \rho) F_N(\vlod \mid \log \hat{v}_i(t), \sigma_v^2) & \text{if $y_{i,t} = 0$} \label{ATACCC:eq:hakki-likelihood}
\end{cases}
\end{align}
...
Because the individual trajectories are not of interest for this analysis, and the population-level parameters agree between the two sets of chains, I consider this lack of convergence acceptable.
\begin{figure}
