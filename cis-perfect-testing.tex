...
A necessary condition for it to perform well, satisfied when the interval lengths are near-constant, is $p(\text{episode $j$ detected} \mid b_j \in [l_j^{(b)}, r_j^{(b)}]) \approx p(\text{episode $j$ detected})$.
...
In this section, I develop a computationally feasible statistical model accounting for double interval censoring and undetected episodes.
...
I consider the cohort of $\Ncis$ individuals who could have had a detected episode; this is a subset of the whole CIS cohort.
...
Any infection episode, whether detected or not, is fully characterised for my purposes by the triplet $(b_j, e_j, i(j))$ where $b_j$ and $e_j$ are the times the episode begin and end respectively, and $i(j)$ denotes the individual the episode occurred in.
...
Therefore, I present this version for clarity.
...
For such an individual, let $j(i)$ denote their detected episode.
...
That is, the prior used in this analysis should have correlation between $\lambda_t$ and $\lambda_{t'}$.
