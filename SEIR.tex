...
\Cref{SEIR:sec:inference-implementation} describes how I will apply the model to the CIS prevalence data, including the inference process, parameterisation, and priors for this model.
\Cref{SEIR:sec:sim-study} checks this recovers the parameters successfully using a simulation study.
\Cref{SEIR:sec:application} then describes the results with the CIS data, including making comparisons to the results in \cref{E-backcalc}.
...
\Cref{SEIR:sec:transmission-generic} explains only the parts relevant to the remainder of the chapter.
...
\todo[inline]{I think this intro needs references but I'm struggling to find suitable ones}
...
\Cref{SEIR:sec:SIR} introduces the \emph{susceptible-infected-recovered} (SIR) model, a simple compartmental model for a disease where individuals are immune to reinfection following their recovery.
...
\begin{itemize}
    \item For any short period of time when $s(t)$ is approximately constant, the system of ODEs can be solved analytically to produce exponential growth\todo{cite exponential growth}.
...
The new compartments are considered biologically identical here.
...
\todo[inline]{add structured pop refs}
...
I use the following age-based strata, which align closely with the strata previously used for analysis of the CIS data\todo{cite analyses using these strata}.
\begin{enumerate}
...
I will use the parameterisation of \textcite{birrellRealtime}, with the decomposition $\beta_{aa'}(t) = \beta(t) \kappa_{aa'}(t) q_{aa'}$ to separate out contact patterns, $\kappa_{aa'}$, which can vary over time; transmission probabilities $q_{aa'}$, assumed constant; and a random effect for time $\beta(t)$.

I take values for the contact rates, $\kappa_{aa'}(t)$, from other data sources.
I use previously produced estimates of these rates based on time use data, and mobility data~\autocites{vanleeuwenTime}{vanleeuwenAugmenting}; these estimates have been used for previous modelling of the COVID-19 pandemic~\autocites{vanleeuwenTime}{birrellRealtime}.
% These start with estimates of contact in each setting, based on \textcite{vanleeuwenAugmenting}.
Baseline (pre-pandemic) estimates of contacts in various settings are taken from the POLYMOD study~\autocite{mossongSocial} and UK Time Use Survey (UKTUS)~\autocite{UKTUS}.
...
These estimates of contacts by setting and then recombined across settings to from $\kappa_{aa'}$
...
To allow for these changes and other model misspecficiation, I follow \textcite{birrellRealtime} in placing a random walk prior on $\beta(t)$ in log space on the weekly timescale:
\begin{align}
    \beta(t) &= \beta_{w(t)} \\
...
The remaining parameters $\beta_1$, the initial random of the random walk, and $\sigma_\epsilon$, the standard deviation of the random walk, will have priors placed on them, with their values inferred in the usual way.
...
I decompose it into two independent components such that $q_{aa'} = q^\text{susc}_{a} q^\text{inf}_{a'}$.
$q^\text{susc}_a$ is the \emph{susceptibility} of stratum $a$, \ie how vulnerable to infection individuals in stratum $a$ are.
$q^\text{inf}_a$ is the \emph{transmissibility} of stratum $a$, \ie how likely individuals in stratum $a$ are to pass on an infection.
...
Previous studies for pre-Alpha SARS-CoV-2 suggest that all individuals have similar infectiousness but that pre-adolescent children are less susceptible than adults\todo{cite where age difference in transmission comes from}.
Therefore, I set $\beta^\text{inf}_{a} = 1$ for all $a$, $\beta^\text{susc}_{a} = q_c$ for the youngest stratum, and $\beta^\text{susc}_{a} = 1$ for the remaining strata.
...
When $\vec{s}$ is approximately constant, and hence the epidemic grows exponentially (see \cref{SEIR:sec:SIR}), the $\vec{e}$, $\vec{i}$ and $\vec{p}$ compartments all grow at the same initial exponential growth rate, $\psi$.
If this is the initial exponential phase, when $\vec{s} \approx 1$, then the basic reproduction number for a SEIR model can be linked to this growth rate by~\autocites{birrellBayesian}{wearingAppropriate}:\begin{align}
    \R = \frac{\psi \left( \frac{\psi}{2\sigma} + 1 \right)^2}{\gamma \left(1 - \frac{1}{\left(\frac{\psi}{2 \gamma} + 1 \right)^2} \right)} \label{SEIR:eq:rtoR}.
\end{align}

Furthermore, $\R$ is linked to $\beta_1$ via $\matr{\beta}$ (see \cref{SEIR:sec:structured-populations}).
This implies that the model can be parameterised by using $\psi$ in place of $\beta_1$.
The growth rate $\psi$ is easily identifiable from the data, and therefore this parameterisation should be more efficient.
The following procedure gives $\beta_1$ in terms of $\psi$.
First, calculate $\R$ from $\psi$ using \cref{SEIR:eq:rtoR}; denote this $\hat{R}$.
Then calculate $R^* = \rho(\matr{\beta}) / \gamma$ when $\beta_1=1$ (recalling that $\rho(\matr{\beta})$ denotes the dominant eigenvalues of $\matr{\beta}$).
...
Now introduce an additional vector of parameters, $\vec{\pi}$, where $\pi_i$ is the proportion of strata $i$ without immunity at time 0.
That is, at time 0, $\vec{r} = 1 - \vec{\pi}$.

From the system of ODEs\todo{might need to reorder this because I've not presented these yet.. } and the knowledge that all the compartments are growing exponentially at rate $\psi$, one can derive the following for the initial state of the compartments:
\begin{align}
...
\begin{tabular}{l c l l}
    ...
    Standard deviation of random walk steps on log scale & $\sigma_\epsilon$ & $\Exponential(80)$ & Weakly informative \\ %penalised model complexity prior~\autocite{simpsonPenalising} \\
    Overdispersion of observations & $\rho$ & $\Exponential(2 \times 10^5)$ & Weakly informative
\end{tabular}
\todo[inline]{Dani/Paul: I think I took the prior for $\beta_c$ from RTM, but can't find the reference now - any ideas?}
...
\todo[inline]{check likelihood is mentioned somewhere in this section}
...
\todo[inline]{following paragraph needs rewording having reordered this chapter}
...
The coverage of the posterior credible intervals should be their nominal value (\eg 95\% of the 95\% credible intervals should contain the true paramter value).
...
There are a few features of the data which are not captured by the model though.
...
\begin{figure}
    ...
    \captionof{figure}[Posterior predictive prevalence (younger ages)]{%
        Posterior predictive prevalence by region and age for the younger three age groups (see \cref{SEIR:fig:prev-old} for the others).
...
\begin{figure}
    ...
    \captionsetup{width=0.8\paperwidth}
    \captionof{figure}[Posterior predictive prevalence (older ages)]{%
        Posterior predictive prevalence for the older three age groups (see \cref{SEIR:fig:prev-young} for the other age groups and details).
    }
...
This is because the parameters within $\vec{\lambda}(t)$ are constant within each week.
A constant $\vec{\lambda}(t)$ leads to exponential growth over short time periods, once the initial dynamics have died out.
...
Parameterisation of this model is limited by the inference techniques.
The algorithm used here tends to struggle with a large number of parameters.
Alternative proposal schemes, such as blocking\todo{cite what RTM does?}, could help, or a more advanced algorithm such as NUTS (used elsewhere in this thesis).

The current model is not very flexible with regards to the age distribution of infections.
The lack of flexibility is visible in \cref{SEIR:fig:prev-young} where how well the model reproduces the data varies by age group.
In some regions, there appears to be a trade-off between the 11--16s and 16--25s, with the model misfitting in opposite directions.
The age distribution of infections determined by $\matr{\beta}$, and to a lesser extent $\vec{\pi}$, the former of which only contains one inferred parameter, $\beta_c$.
More parameters would help to capture these dynamics more accurately.
Increasing the number of parameters inferred in the contact matrices would allow for more flexibility in the age distribution of infections, either by introducing additional susceptibility or infectiousness parameters, or by allowing these parameters to vary over time.
For example, the introduction and relaxation of control measures within schools over time is a plausible mechanism changing the age distribution of infections that cannot be captured in the current model because it only uses school attendance data with no allowance for the changing risk of within-school contacts.
...
These factors are likely to be correlated across the country, giving the similar patterns for different regions (see \cref{SEIR:fig:beta-walk}).
Exploiting these correlations by linking their values across regions could improve the precision of the estimates.
This would require a more complex model without the independence of parameters between regions.

Various factors could explain the pattern in the $\beta_t$ values, which have a minima near the start of December.
Some hypothesis follow, although this analysis is unable to distinguish causality.
Increased caution as cases and deaths increased could explain the decrease (\eg masking or stricter adherence to social distancing).
The increase coincides with the rise of the Alpha variant, known to be more transmissible.
This is a likely cause of the increase.
A multiple variant model might be able to capture this more accurately.
This would stratify positive tests in CIS by pre-Alpha or Alpha variant.
...
However, this requires a more complex model structure.
...
\todo[inline]{move the following discussion to conclusion chapter as relevant to whole thesis}
