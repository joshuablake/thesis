...
The backcalculation approach makes no assumptions about the transmission process; the assumptions have the potential to introduce bias.
...
\subsection{Approach} \label{backcalc:sec:approach}
...
This both helps stabilise the estimation (see \cref{intro:sec:disease-model}) and allows borrowing of information across strata.
...
\begin{align}
    \vec{P} \sim \vec{\phi} &\dist g_P(\vec{\phi}) \\
    Y_{i,t} &\dist \BB(n_{i,t}, P_{i,t})
\end{align}
where $g_P$ is analogous to $g_Z$ in providing a joint prior for the prevalence in each stratum and day combination.
...
Here I explain that further developments, beyond the scope of this thesis, would be required to use the incidence-inferred approach with a suitable $\pi_Z$ within the computational constraints of the SRS.
...
\begin{enumerate}
    \item The smoothing model over time should penalize changes from continuous exponential growth.
        ...
        Furthermore, such penalization has previously been shown to improve estimates of SARS-CoV-2 prevalence by balancing over and under-fitting~\autocite{ealesAppropriately}.
...
Properties 1 and 2 suggest a model of the form $\logit(\pi_{i,t}) = g(i, t)$, where $g$ is a function with changes in the first derivative penalized (\ie changes away from linear are penalized).
...
Combined with property 4, the form of $g$ the null model is $g(i, t) = a_0 + a_t t + f(i)$ where $a_0$ and $a_t$ are model parameters and $f$ is a function to be specified; that is, changes away from this model should be penalized.
...
In particular, the prior for $\pi_P$ places non-zero mass on prevalence values which imply negative incidence under the other model assumptions.
...
Once the incidence proportion in each stratum have been calculated using this procedure, I poststratify it to the CIS target population and subpopulations of interest.
