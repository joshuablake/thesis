...
In particular, recoveries need considering, which was not the case for HIV/AIDS.
...
The \emph{survival function} gives the probability that the duration is at least $d$ days: $S(d) = \prob(D \geq d) = 1 - F_{D}(D - 1)$ with $S(1) = 1$ (in some of the literature on survival with discrete variables, the convention that the survival function is $\prob(D >d)$, rather than $\prob(D \geq d)$, is used).
The \emph{hazard} on day $d$ is the probability of recovering at time $d$ conditional on having survived to time $d$: $\lambda(d) = \prob(D = d \mid D \geq d) = f_{D}(d) / S(d)$.
Throughout this thesis, the survival function will be the primary quantity of interest, and it will normally be parameterised in terms of the hazard; these two quantities are related by $S(d) = \prod_{i=1}^{d-1} (1 - \lambda(i))$.

Since $D_i$ is a discrete random variable, $f_D(d) \geq 0$ for all $d$ and $\sum_{i=1}^{\dmax} f_D(i) = 1$.
This implies that $F_D(d)$ is monotonically increasing.
Therefore, $S(d)$ is monotonically decreasing.
Both $F_D$ are $S$ are in the interval $[0, 1]$.
$\lambda(d)$ is in the interval $[0, 1]$, and can otherwise vary arbitrarily (\ie there is no monotonic or sum constraint).
...
Therefore, overdispersed epidemics cannot be modelled by a Poisson process.
...
The independence assumption of a Poisson or negative binomial process is violated by an infectious disease because stochastically high (or low) or low incidence, feeds back to the process intensity at future times.
...
The proposal distribution is most commonly a (possibly multivariate) Normal distribution centred on $\psi$, this is used in \cref{E-SEIR}.
...
The full Metropolis-Hastings algorithm is given in \cref{inc-prev:algorithm:MH}, with further details in available in a wide-range of sources~\autocites[e.g.][]{brooksMCMCNotes}[chapter 11]{gelmanBDA}.
\begin{algorithm}
...
\emph{Hamiltonian Monte Carlo} (HMC)~\autocites{nealMCMC}{nealImproved}{duaneHybrid} improves the proposals by using information from the gradient of the posterior density (with respect to the parameters).
...
The \emph{No U-Turn Sampler} (NUTS)~\autocite{hoffmanNUTS} is a variant of HMC that automatically tunes the number of steps to take in the Hamiltonian dynamics.
