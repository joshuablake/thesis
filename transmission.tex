...
The backcalculation approach makes no assumptions about how transmission occurs, which have the potential to introduce bias.
...
\begin{align}
    \vec{Z} \mid \vec{\phi} &\dist g_Z(\vec{\phi}) \\
    \vec{P_{i}} &= \matr{S} \vec{Z}_i \\
    Y_{i,t} \mid \vec{P} &\dist \BB(n_{i,t}, P_{i,t})
\end{align}
where $\vec{Z}$ is a vector of incidence proportion for each strata and day defined by some model $g$ with parameters $\vec{\phi}$; $P_{i,t}$ is the prevalence in stratum $i$ on day $t$; $\matr{S}$ is the survival matrix defined in \cref{E-inc-prev:eq:Smatrix}; $Y_{i,t}$ is the number of positive tests in stratum $i$ on day $t$; and $n_{i,t}$ is the number of tests in stratum $i$ on day $t$.
The model $g_Z$ would be specified to share information across time and/or strata.
This both helps stabilise the estimation (see\todo{ref wherever I end up explaining backcalculation stability issues}) and allows borrowing of information across strata.
Borrowing information is required because some strata and time combinations have few positives (fewer than five per day).
The relationship between $P$ and $Z$ is from \cref{E-inc-prev:eq:EPt-matrix}, although dividing both sides by $\Npop$ to have the values in terms of incidence proportion and prevalence rather than incidence and number of prevalent individuals.
...
Then, it transforms the posterior distribution of prevalence to a posterior distribution of incidence using the forward substitution as in \cref{E-inc-prev:eq:forward-substitute}.
At a high-level, the statistical model is:
\begin{align}
    \vec{P} \mid \vec{\phi} &\dist g_P(\vec{\phi}) \\
    Y_{i,t} \mid \vec{P} &\dist \BB(n_{i,t}, P_{i,t})
\end{align}
where $g_P$ is analogous to $g_Z$ in providing a joint prior for the prevalence in each strata and day combination.
Step 2 is...
...
Here I explain that further developments, beyond the scope of this thesis, would be required to use the incidence-inferred approach with a suitable $g_Z$ within the computational constraints of the SRS.

Denote the quantity to be inferred at time $t$ in strata $i$ as $\pi_{i,t}$.
That is, under the incidence-inferred approach, $\pi_{i,t}$ is the incidence proportion, and under the prevalence-inferred approach, $\pi_{i,t}$ is the prevalence.

The chosen approach should have the following properties.
\begin{enumerate}
    \item The smoothing model, $g_Z$ or $g_P$, over time should penalise changes from continuous exponential growth of $\pi$ over time.
        ...
        Furthermore, such penalisation has previously been shown to improve estimates of SARS-CoV-2 prevalence by balancing over and under-fitting~\autocite{ealesAppropriately}. Used by CIS (section 3.?).
    ...
    \item Borrow information across the different covariates.
        The borrowing should penalise changes in the distribution of incidence/prevalence across strata in the same geographic region being constant.
...
Properties 1 and 2 suggest a model of the form $\logit(\pi_{i,t}) = g(i, t)$, where $g$ is a function with changes in the first derivative penalised w.r.t.\ $t$ (\ie changes away from linear are penalised).
...
If $\eta_j = \logit(\pi_{i,t})$, where $\pi$ is prevalence, then a model of $\eta_j = g(i, t)$, as before, is a LGM.
...
\end{align}
As $g(i, k)$ is linear in the model parameters, $\vec{z}$, then $\eta_{i,t}$ is not, violating the requirements of a LGM.

Therefore, using the INLA approximation while satisfying the other desirable properties requires that $g$ predicts prevalence, not incidence proportion.
Hence, I adopt the prevalence-inferred approach.
...
In this context, that means the prior $\pi_P$ places non-zero mass on prevalence values which imply negative incidence under the other model assumptions.
...
For example, a Laplace approximation or variational inference.
Further work, to check the quality of these approximations, is required.
...
The largest modification from the generic framework presented in \cref{E-SEIR} is the parameterisation of the WAIFW matrix (\cref{SEIR:sec:WAIFW-application}).
...
\begin{figure}
    ...
    \todo[inline]{Fix x-axis labels overlapping / being unreadable}
\end{figure}
...
\begin{figure}
    ...
    \todo[inline]{Remake/reposition this plot so that it fits on a page.}
\end{figure}
