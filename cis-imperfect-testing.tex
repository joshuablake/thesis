...
Denote the set of the remaining test times to consider as $\sched'_{i(j)}$, the set of times individual $i(j)$ was tested between $l_j^{(b)}$ and $r_j^{(e)} + 1$ inclusive.
...
This gives $t_+ = \sum_{t \in \sched'_{i(j)}} y_{i(j)}(t)$ true positives and $f_- = \sum_{t \in \sched'_{i(j)}} (1 - y_{i(j)}(t))$ false negatives.
...
First, the case when $E_j \leq r_j^{(e)}$.
In this case, the test at $r_j^{(e)}+1$ is a true negative and the end of the episode is interval censored as in the previous chapter.
...
&= p_\text{sens}^{t_+} (1 - p_\text{sens})^{f_-} \left( S_{\vec{\theta}}(l_j^{(e)} - b + 1) - S_{\vec{\theta}}(r_j^{(e)} - b + 2) \right)
\label{imperf-test:eq:ll-ei-lt-ri}
\end{align}

Second, the case when $E_j > r_j^{(e)}$.
...
&= p_\text{sens}^{t_+} (1 - p_\text{sens})^{f_-} (1 - p_\text{sens}) S_{\vec{\theta}}(r_j^{(e)} - b_j + 2)
\label{imperf-test:eq:ll-ei-gt-ri}
\end{align}

These expressions can now be used to form the full replacement for $p_{ia}$, $p'_{ia}$, the probability of observing $\vec{y_j}$ given that an infection occurs in individual $i$.
First, augment the data with $b_j$, and split into the cases just discussed:
\begin{align}
p_{ia}'
=& \prob(\vec{y_j} \mid p_\text{sens}, \vec{\theta}) \\
=& \sum_{b = l_j^{(b)}}^{r_{j(i)}^{(b)}} \left( \prob(\vec{y_j}, E_j \leq r_{j(i)}^{(e)} \mid B_j = b, p_\text{sens}, \vec{\theta}) + \prob(\vec{y_j}, E_j > r_{j(i)}^{(e)} \mid B_j = b, p_\text{sens}, \vec{\theta}) \right) \\
...
=\ &  p_\text{sens}^{t_+} (1 - p_\text{sens})^{f_-} \\
 & \times \sum_{b = l_j^{(b)}}^{r_{j(i)}^{(b)}} \left( S_{\vec{\theta}}(l_j^{(e)} - b + 1) - S_{\vec{\theta}}(r_{j(i)}^{(e)} - b + 2) + (1 - p_\text{sens}) S_{\vec{\theta}}(r_{j(i)}^{(e)} - b + 2) \right) \\ 
  & \times \prob(B_j = b \mid p_\text{sens}, \vec{\theta}) \\
=\ &  p_\text{sens}^{t_+} (1 - p_\text{sens})^{f_-} \\
  & \times \sum_{b_j = l_j^{(b)}}^{r_{j(i)}^{(b)}} \left( S_{\vec{\theta}}(l_j^{(e)} - b_j + 1) - p_\text{sens} S_{\vec{\theta}}(r_{j(i)}^{(e)} - b_j + 2) \right) \\
  & \times \prob(B_j = b \mid p_\text{sens}, \vec{\theta}).
...
\begin{align}
p_{ia}'
&\propto \sum_{b = l_j^{(b)}}^{r_{j(i)}^{(b)}} S_{\vec{\theta}}(l_j^{(e)} - b_j + 1) - p_\text{sens} S_{\vec{\theta}}(r_{j(i)}^{(e)} - b + 2).
...
\begin{enumerate}
    \item The episode would have been detected considering only the mechanisms in \cref{E-perf-test:sec:prob-undetected}. That is $\min(\sched_{i(j)}) < b_j \leq \max(\sched_{i(j)})$ and $e_j \geq \tau_{\sched_{i(j)}}(b_j) + b_j$.
...
\begin{align}
&\prob \left(
    \tau_{\sched_{i(j)}}(b_j) + 1 \leq D_j \leq \tau^2_{\sched_{i(j)}}(b_j) + 1, b_j \leq \max(\sched_{i(j)}),
    \mid B_j = b, \vec{\theta} \right) (1 - \psens) \\
&= \left( S_{\vec{\theta}}(\tau_{\sched_{i(j)}}(b_j) + 1) - S_{\vec{\theta}}(\tau^2_{\sched_{i(j)}}(b_j) + 2) \right) (1 - \psens).
\end{align}
...
\begin{align}
\zeta = (1 - p_\text{sens})\frac{1}{T} \sum_{b=\min(\sched_{i(j)}) + 1}^T \left( S_{\vec{\theta}}(\tau_{\sched_{i(j)}}(b) + 1) - S_{\vec{\theta}}(\tau^2_{\sched_{i(j)}}(b) + 2) \right).
\end{align}
