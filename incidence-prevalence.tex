...
I then move on to explaining the three processes that link the incidence, prevalence, and the data: the \emph{infection process}, the \emph{prevalence process}, and the \emph{observation process}.

In (\cref{inc-prev:sec:infection-process}) I introduce the infection process, describing the infections' generating mechanism; in (\cref{inc-prev:sec:prevalence-process}) the prevalence process, which generates prevalence from the incidence; and in (\cref{inc-prev:sec:observation-process}), the observation process, which generates the data from the prevalence through the sampling mechanism of a population prevalence survey, guided by the design of the CIS.

\Cref{inc-prev:sec:conclusion} concludes the chapter.

\section{Definitions} \label{inc-prev:sec:definitions}

I use discrete time in this chapter, except in \cref{SEIR:sec:mechanistic-models}, with the time unit of a day. However, results generalise in the obvious way to other time units.
...
The distribution of $D_i$ can be expressed through the standard probability mass function (pmf), $f_{D}(d) = \prob(D = d)$; or cumulative density function (CDF), $F_{D}(d) = \prob(D \leq d) = \sum_{i=1}^d f_{D}(i)$.
...
The lack of monotonicity or sum constraint on the hazard makes it an attractive parameterization for inference because these constraints are challenging to implement in many inference frameworks~\autocite{heBayesian}.
...
It makes no explicit assumptions on the underlying biological or epidemiological mechanism and can be a flexible model.
...
Deterministic models are defined by the state vector $\vec{x}(t)$ being a deterministic function of the parameters $\vec{\theta}$ and the initial state $\vec{x}(0)$; equivalently, $\var(\vec{x}(t) \mid \vec{x}(0), \theta) = \matr{0}$~\autocite{birrellEvidence}.
...
\begin{itemize}
    ...
    \item A discretised incidence proportion for day $t$ can be defined as $Z_t / \Npop = s(t) - s(t+1)$.
...
\end{align}
In the CIS $n_t << \Npop$ because $\Npop$ is the population of England (around 56 million) while $n_t$ is the daily sample size (10s of thousands).
Therefore:
\begin{align}
