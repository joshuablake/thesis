...
\Textcite{shenNonparametrica}, extending \textcite{panNote}, showed that the maximum likelihood estimator is inconsistent and can lead to a severely biased underestimation of the survival function, albeit when only the terminating event is interval censored\todo{double check the setting of Shen}.
...
\todo[inline]{Check all this notation with the previous chapter's}
\begin{enumerate}
\item
...
I repeated the simulation study in \cref{E-perf-test:sec:sim-study}, except I used the model of \cref{imperf-test:sec:modelling} with either a constant test sensitivity or the more realistic varying sensitivity.
...
\begin{itemize}
    \item not sensitive to survival prior
