...
The CIS sampling is less frequent, but with has a large sample size (\numprint{4800} detected infection episodes) and a long follow-up.
...
The model will be used to analyse the 4,800 episodes first detected between 10th Oct 2020 and 6th Dec 2020 inclusive, with negatives bounding the start and end time of the episode.
...
Let $i = 1, \dots, \Ncis$ index individuals and $j = 1, \dots \ntot$ index infection episodes.
...
All the relevant information about the infection episode $j$ can be fully characterized by the (unobserved) triplet $(b_j, e_j, i(j))$ where $b_j$ and $e_j$ are the times the episode begin and end respectively and $i(j)$ denotes the individual the episode occurred in.
...
I define the precise conditions for this to occur in \cref{perf-test:sec:prob-undetected}.
% That is, $i \in \set{D}$ if and only if there exists a $j$ such that $b_{j}$ and $e_{j}$ such that $(b_{j}, e_{j}, \sched_i)$ is a detected episode. 
...
Each episode, whether detected or not, must fall into one of these three classes with a probability that is a function of the parameters of the survival function, $\vec{\vec{\theta}}$, discussed in \cref{perf-test:sec:parameters-priors}.
...
\item
  ...
  Conditional on the episode occurring in individuals with test schedule $\sched_i$, the episode is undetected with probability $p_{iu} = \prob((B_j, E_j) \in \Omega_i \mid \vec{\theta}, i(j) = i)$.
\item
...
This represents all our observations.
...
From \cref{perf-test:sec:episode-types}, counts $\na$ and $\nnodet$ conditional on $\ntot$ are jointly distributed multinomially.
...
\subsection{Expression for the sum}
...
\begin{enumerate}
    ...
    \item $\exists t \in \sched_{i(j)}$ such that $t < b_j$.
      For individuals enrolled during the period considered ($\min \sched_{i(j)} > 0$), this ensures that the beginning of the episode is lower bounded.
...
% \end{align}
Therefore, suppressing the conditioning on $\vec{\theta}$ and $i(j) = i$:
\begin{align}
1 - p_{iu}
...
\intertext{Substituting into \cref{perf-test:eq:pu}:}
1 - p_u
